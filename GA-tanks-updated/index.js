...
window.onload = () => {
    ...
    window.addEventListener("keypress", (event) => {
        ...
        if (event.keyCode == enums.keyboard.KEY_C + 32) {
            let vec = new Vector(world.mouse.x, world.mouse.y);

            world.checkpoints.push(new Checkpoint(vec));

            world.tanks.forEach(t => {
                t.checkpoints.push(new Checkpoint(vec));
            });
        }
    }, 1);
...
function init() {
    world.geneticModel = new Genetic(50);
    reset();

    for (let i = 0; i < world.geneticModel.populationSize; i++) {
        let params = randomParameters(24);
